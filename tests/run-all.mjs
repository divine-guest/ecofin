...
const SUITES = [
  ...
  ["freshness", "содержание не разошлось со ставками и не устарело молча"],
  ["calendar", "личный налоговый календарь: кому какие сроки и на какую дату"],
  ["redact", "обезличивание: что не уходит за границу и что не ломается"],
  ["e2e", "сквозной путь: регистрация, лимиты, роли, промокод"],
  ["security", "подделка токена, чужой домен, перебор пароля"],
  ["reset", "смена забытого пароля: код из письма, срок и попытки"],
  ...
  ["requisites", "реквизиты, контрагенты и нумерация документов"],
  ["documents", "мои документы: история, оплата и связь с учётом"],
  ["reminders-limit", "бесплатные места под сроки: кто кого вытесняет"],
  ["erasure", "удаление аккаунта стирает всё, выгрузка отдаёт всё"],
