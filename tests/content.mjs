...
console.log("\n— Распознавание: порядок путей —");
{
  ...
  const posForeign = body.indexOf("callProvider(env");

  ok(posYandex > -1 && posLocal > -1 && posForeign > -1, "все три пути на месте");
  ok(posYandex < posLocal, "российский OCR пробуется раньше локального");
  ok(posLocal < posForeign, "локальный — раньше зарубежной модели",
     [posLocal, posForeign]);
