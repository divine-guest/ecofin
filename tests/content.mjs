...
console.log("\n— Распознавание: порядок путей —");
{
  ...
  /* Третьего пути больше нет. Он вёл в зарубежную зрячую модель и
     срабатывал только на машине без tesseract — то есть на боевой
     не срабатывал никогда. Убрали не поэтому: политика обещает, что
     изображения страну не покидают, и обещание с исключением в коде
     превращается в неверное утверждение в документе, поданном
     в Роскомнадзор. */
  ok(body.indexOf("callProvider(env") === -1,
     "зарубежного пути для снимков не осталось");
  ok(/throw new Error/.test(body),
     "вместо него — честный отказ");

  /* Пустой результат локального распознавания не должен тихо
     проваливаться к зарубежной модели: политика обещает, что
     распознавание идёт внутри страны. */
  ok(/if \(text\.trim\(\)\) return \{ text, where: "local" \}/.test(body),
     "локальный путь отдаёт результат, а не проваливается дальше");
  ok(/throw new Error\("Не удалось разобрать текст на снимке/.test(body),
     "при пустом распознавании — честная ошибка, а не отправка за границу");
...
console.log("\n— Выключатели передачи за границу —");
{
  ...
  for (const name of ["AI_ABROAD_OFF", "TELEGRAM_OFF"]) {
    ...
    ok(/выключен\w* с \d{2}\.\d{2}\.\d{4}/.test(tail),
       `${name} включён и объяснён с датой`, tail.slice(-120));
    ok(/Включать обратно|включить обратно/i.test(tail),
