...
console.log("\n— Цены на витрине —");
{
  /* Цену человек видит до оплаты, а списывается та, что в plans.js.
     Разойтись они могут молча: витрину правят руками, тариф — в коде.
     Поэтому сверяем разметку главной с источником правды.

     Отдельно проверяем, что цифры вообще есть в разметке: раньше
     блок рисовался только скриптом, и без ответа сервера на месте цен
     оставалось «Загружаем тарифы…» — этого не видит ни поисковик,
     ни модератор платёжного сервиса. */
  const { PLANS } = await import("../worker/src/plans.js");
  const home = read("../index.html");
  const money = n => n.toLocaleString("ru-RU").replace(/\u00a0/g, " ");

  for (const p of Object.values(PLANS)) {
    if (!p.price.month) continue;
    ok(home.includes(`${money(p.price.month)} ₽`),
       `цена «${p.title}» есть в разметке главной: ${money(p.price.month)} ₽`);
    ok(home.includes(`${money(p.price.year)} ₽ за год`),
       `годовая цена «${p.title}» есть в разметке: ${money(p.price.year)} ₽`);
  }
  ok(!home.includes("Загружаем тарифы"), "на месте цен нет заглушки");
  ok(/Что происходит после оплаты[\s\S]{0,600}личном кабинете/.test(home),
     "сказано, как человек получает оплаченное");
  ok(/Что происходит после оплаты[\s\S]{0,900}legal\.html/.test(home),
     "рядом стоит ссылка на оферту");
}
