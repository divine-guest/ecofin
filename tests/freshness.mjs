...
console.log("\n— Ставки в текстах совпадают с расчётными —");
{
  ...
  /* В текстах дробное пишется через запятую: «490,5 млн». Через точку
     это число не найдётся, и проверка соврёт, что статьи отстали.
     Для лимита самозанятости такое преобразование уже делалось строкой
     выше — здесь его просто забыли, и вылезло это только тогда, когда
     предел УСН впервые стал дробным. */
  /* Имя не «ru»: такая функция уже объявлена выше в этом же файле, и
     объявление внутри блока перекрыло бы её для всего блока — включая
     строки выше по тексту. Получилась бы «Cannot access before
     initialization» в проверках, которых правка не касалась. */
  const comma = n => String(n).replace(".", ",");
  ok(allText.includes(comma(vatMln) + " млн"),
     `порог НДС на УСН (${comma(vatMln)} млн) встречается в статьях`);
  ok(allText.includes(comma(limMln) + " млн"),
     `предел УСН (${comma(limMln)} млн) встречается в статьях`);
