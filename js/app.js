...
/* ============ Тарифы и оплата ============
   ...
   за 490 старший тариф выглядит выбором, а не преградой, и человек
...
const SETTINGS = {
  ...
  async loadTelegram() {
    ...
    try {
      ...
      /* Что именно придёт — сказано до подключения, а не после.

         В сводке недели есть и предложения по подписке, а сообщение о
         платной услуге, разосланное типовым текстом, надзорный орган
         считает рекламой. Реклама по сетям электросвязи допускается
         только с предварительного согласия (ст. 18 38-ФЗ), и согласие
         это должно быть осознанным: человек должен знать, на что
         соглашается, до того как нажал. */
      box.innerHTML = `
        ...
        <p class="hint">Подключив мессенджер, вы соглашаетесь получать напоминания
          о сроках, сводку недели и сообщения о тарифах сервиса. Отключить —
          там же, одной кнопкой, в любой момент.</p>`;
    } catch (e) {
...
/* ============ Пейволл (окно подписки) ============
   ...
   — говорила «Нужна подписка «Про»», хотя тарифов три, а почти все упоры
     снимает «Базовый» за 490 ₽ — и вела к самому дорогому;
...
async function paywallFill(reason, need) {
  ...
  /* По умолчанию показываем самый дешёвый платный: разговор должен
     начинаться с 490 ₽, а не с 890 — иначе человек не берёт ни то, ни то. */
