...
/* ============ ЖУРНАЛ СВЕРКИ ============
   ...
   Дата checkedOn ниже говорит «сверяли тогда-то», но не говорит, ЧТО
   именно сверяли. Из-за этого мимо файла прошла вся налоговая реформа
   2026 года при совершенно свежей дате.

   Поэтому здесь — поимённо.

   СВЕРЕНО 09.09.2026 по тексту законов и разъяснениям ФНС:

   • НДС 22% и порог НДС на УСН 20 млн — ФЗ от 28.11.2025 № 425-ФЗ,
     порог заморожен до конца 2029 ФЗ от 04.07.2026 № 228-ФЗ;
   • предел УСН 490,5 млн и границы ставок 5/7% (272,5 млн) —
     базовые значения НК с коэффициентом-дефлятором 1,090;
   • лимит ПСН 20 млн — тот же 425-ФЗ;
   • льгота МСП по взносам: порог 1,5 МРОТ и три условия права —
     425-ФЗ и распоряжение Правительства от 27.12.2025 № 4125-р;
   • взносы ИП за себя 57 390 и потолок 321 818;
   • МРОТ 27 093, предельная база взносов 2 979 000;
   • шкала НДФЛ 13-15-18-20-22 и пороги 2,4 / 5 / 20 / 50 млн;
   • детские вычеты 1400 / 2800 / 6000 / 12000, предел дохода 450 000;
   • социальный вычет 150 000, обучение детей 110 000, имущественный
     2 000 000, проценты по ипотеке 3 000 000, ИИС 400 000;
   • предельные базы больничного за 2024 и 2025;
   • декрет 140 / 156 / 194 дня, пособие по уходу 40%;
   • максимальная ключевая ставка за 2026 год — 16% (для вклада);
   • ключевая ставка 14%;
   • штрафы: ст. 119 (5% в месяц, до 30%, минимум 1000), ст. 122
     (20% и 40%), смягчение вдвое по ст. 114;
   • продажа имущества: сроки 5 и 3 года, вычеты 1 000 000 и 250 000;
   • алименты 1/4, 1/3, 1/2 и предел удержания 70%;
   • исковая давность 3 года, предельная 10 лет, приостановка 180 дней.

   СВЕРЕНО 11.09.2026 по тексту статьи в действующей редакции (Гарант):

   • госпошлина в суд общей юрисдикции (ст. 333.19): все ступени от
     4 000 ₽ до 100 тыс. и выше, потолок 900 000 ₽, неимущественный иск
     физлица 3 000 ₽. Пункт долго висел «не подтверждён», потому что
     справочные сайты показывали дореформенный текст.

   СВЕРЕНО 09.09.2026, продолжение:

   • НПД: лимит 2,4 млн и ставки 4% и 6% — не менялись, режим продлён
     как минимум до конца 2028 года;
   • АУСН: лимит 60 млн закреплён до конца 2027 года; фиксированный
     взнос на травматизм в 2026 году — 2 959 ₽ (стояло 2 750 —
     прошлогоднее значение, найдено при сверке статей 11.09.2026);
   • ЕСХН: ставка 6% и порог 60 млн. НО с 2026 освобождение от НДС
     больше не автоматическое — только по уведомлению (ст. 145 НК);
   • ОСНО: налог на прибыль 25%, профвычет ИП 20%;
   • налог на имущество физлиц: вычеты 10 / 20 / 50 м², надбавка
     многодетным, ставки 0,1% / 0,3% / 2% от 300 млн;
   • транспортный налог: базовые ставки ст. 361, право региона менять
     их в десять раз, повышающий коэффициент 3 от 10 млн;
   • выходное пособие: пособие за первый месяц, заработок за второй,
     если не устроился, за третий — по решению службы занятости.
     ИСПРАВЛЕНО 11.09.2026: калькулятор брал месяцы без работы
     целиком и при двух месяцах давал три заработка вместо двух;
   • классы профессионального риска: от 0,2% до 8,5%.

   ЕЩЁ НЕ СВЕРЕНО: любое значение, которого нет в этом журнале.
   Добавили число в файл — добавьте сюда строку, откуда оно взято.

   СТАТЬИ БАЗЫ ЗНАНИЙ прочитаны целиком и сверены с кодексами
   10–11.09.2026. Там не числа, а нормы, и поиском по цифрам такие
   ошибки не находятся: «Коммерсант» при банкротстве гражданина,
   четыре года хранения вместо пяти, «один тип» ИИС, штраф 50%
   «только с претензией» — всё это читалось правдоподобно.

   Правило простое: сверять по тексту закона, а не по памяти и не по
   первой попавшейся статье. Дважды за один день справочные сайты
   показывали устаревшие нормы как действующие.                     */
...
const RATES = {
  ...
  ausn: {
    ...
    workersLimit: 5,
    injuryFixed: 2959,       // фиксированный взнос на травматизм за год
  },
  ...
  eshn: {
    rate: 0.06,
    /* Порог освобождения от НДС прежний — 60 млн. Изменилось другое:
       с 2026 года освобождение больше НЕ ДАЁТСЯ автоматически. Раньше
       плательщик ЕСХН просто не платил НДС, пока укладывался в порог;
       теперь нужно самому оценить право по статье 145 НК и подать
       уведомление.

       Число то же, а поведение противоположное: кто ничего не сделал,
       тот стал плательщиком НДС, не заметив этого. */
    vatExemptUpTo: 60000000, // порог освобождения по ст. 145 НК
    vatExemptByNotice: true, // с 2026 — только по уведомлению, не само
  },
  ...
  severance: {
    ...
    extraMonths: 2,         // второй месяц — если не устроился, третий — по решению службы занятости
  ...
  severancePay({ avgMonth = 0, monthsUnemployed = 0 } = {}) {
    ...
    /* Первый месяц после увольнения покрывает само пособие. Заработок
       «на время поиска» платят со ВТОРОГО месяца: за второй — если
       человек не устроился (за неполный — пропорционально), за третий —
       по решению службы занятости.

       До 11.09.2026 здесь месяцы без работы брались целиком, и два
       месяца без работы давали три заработка вместо двух: калькулятор
       завышал итог на целую зарплату. */
    const extra = Math.max(0, Math.min(this.severance.extraMonths, monthsUnemployed - 1));
    return {
