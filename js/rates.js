...
const RATES = {
  ...
  usn: {
    ...
    /* Лимиты индексируются на коэффициент-дефлятор: на 2026 год он
       1,090, поэтому базовые 450 млн из НК превращаются в 490,5 млн.
       Держать здесь базовое значение — значит занижать предел почти
       на сорок миллионов и говорить человеку, что он слетел с УСН,
       когда он на ней остался. */
    limit: 490500000,      // предел для применения УСН в 2026 (450 млн × 1,090)

    /* Порог НДС снижен с 60 до 20 млн Федеральным законом от
       28.11.2025 № 425-ФЗ. Планировали снижать дальше — до 15 млн
       в 2027 и 10 млн в 2028, — но ФЗ от 04.07.2026 № 228-ФЗ
       заморозил 20 млн до конца 2029 года.

       Это самое дорогое число в файле. Ошибка в нём означает, что
       человек с выручкой 25 млн не выставлял НДС весь год. */
    vatThreshold: 20000000,// с этого дохода на УСН появляется НДС
  },
  ...
  psn: {
    rate: 0.06,
    /* Лимит снижен с 60 до 20 млн с 01.01.2026 (ФЗ от 28.11.2025
       № 425-ФЗ) — сразу втрое. Дальше по тому же закону 15 млн
       в 2027 и 10 млн с 2028. Превышение лимита прекращает действие
       патента прямо в течение срока. */
    incomeLimit: 20000000,   // лимит фактического дохода в 2026
    workersLimit: 15,
  },
  ...
  /* Основная ставка НДС повышена с 20% до 22% с 01.01.2026
     ...
     По этой ставке считается каждый счёт, выставленный через сервис. */
  vatRate: 0.22,
  ...
  payrollContrib: {
    /* Предельная база меняется каждый год. Не путать с базами для
       больничного выше: там берутся два ПРЕДШЕСТВУЮЩИХ года, и
       значение 2 759 000 там стоит правильно — это 2025-й. */
    base: 2979000,        // предельная база 2026 года
    rate: 0.30,           // единый тариф до базы
    rateOverBase: 0.151,  // сверх базы
    /* Льготный тариф МСП. С 2026 года применяется к части выплаты
       свыше ПОЛУТОРА МРОТ (было — свыше одного), и только для тех,
       чей основной ОКВЭД входит в перечень Правительства
       (распоряжение от 27.12.2025 № 4125-р) и даёт не менее 70%
       дохода. Оба условия ввёл ФЗ от 28.11.2025 № 425-ФЗ. */
    smallRateOverMrot: 0.15,   // МСП: с превышения порога
    smallMrotFactor: 1.5,      // порог: столько МРОТ идёт по полному тарифу
  ...
  vatRates: [
    { value: 0.22, label: "22% — основная" },
    { value: 0.10, label: "10% — продукты, детские товары, лекарства, книги" },
    { value: 0.07, label: "7% — УСН, доход от 272,5 до 490,5 млн" },
    { value: 0.05, label: "5% — УСН, доход от 20 до 272,5 млн" },
    { value: 0, label: "0% — экспорт и международные перевозки" },
  ],

  /* Границы пониженных ставок НДС на УСН, уже с дефлятором 1,090.
     Базовые значения в НК — 250 и 450 млн. */
  usnVatBands: {
    reduced5UpTo: 272500000,
    reduced7UpTo: 490500000,
  },
  ...
  deposits: {
    ...
    /* Максимальная ключевая ставка из действовавших на ПЕРВОЕ ЧИСЛО
       каждого месяца этого года — именно она, а не текущая, определяет
       необлагаемую сумму (п. 1 ст. 214.2 НК РФ).

       Разница появляется, когда ставка снижается. В 2026 году максимум
       на первое число — 16%, а к сентябрю ставка опустилась до 14%.
       По максимуму необлагаемая сумма 160 000 ₽, по текущей вышло бы
       140 000: лишние двадцать тысяч в базе и около 2 600 ₽ лишнего
       налога.

       Это отдельное число, а не вычисление из keyRate: вывести его
       из текущей ставки нельзя в принципе — нужна история за год.
       Пересматривать в январе, когда год закроется. */
    maxKeyRateYear: 16,
  ...
  /* small — есть ли право на льготный тариф МСП. По умолчанию НЕТ,
     и это изменение 2026 года, а не осторожность.

     До конца 2025 льгота была у всех субъектов МСП, и предполагать её
     было верно. С 2026 она только у тех, чей основной ОКВЭД входит в
     перечень Правительства и даёт не менее 70% дохода. Сервис ОКВЭД
     работодателя не знает, а занижать взносы опаснее, чем завышать:
     завышенный расчёт человек перепроверит, потому что жалко денег,
     а заниженный примет с радостью и узнает правду от инспекции. */
  employerContrib(yearSalary, { small = false, injury = 0.002, mrotMonth = null, months = 12 } = {}) {
    ...
    if (small) {
      /* Льгота считается помесячно: полный тариф до порога, 15% с
         остального. Порог с 2026 года — полтора МРОТ, а не один:
         с той половины, что добавилась, теперь берётся 30%, и раньше
         мы её считали по 15%.
      ...
      const threshold = mrot * c.smallMrotFactor;
      const perMonth = yearSalary / m;
      const atFull = Math.min(perMonth, threshold);
      const above = Math.max(0, perMonth - threshold);
  ...
  depositTax({ interest = 0, maxKeyRate = null } = {}) {
    ...
    /* По умолчанию — максимум за год, а не текущая ставка. Раньше
       подставлялась текущая, и при снижении ставки необлагаемая сумма
       выходила заниженной вместе с ней. */
    const rate = (maxKeyRate ?? d.maxKeyRateYear) / 100;
  ...
  /* Ключевая ставка меняется восемь раз в год — чаще всего
     остального в этом файле. Поэтому у неё своя дата сверки, и
     проверка следит за ней строже: значение старше промежутка между
