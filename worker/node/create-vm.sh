...
# закрыто всё.
# Какой из закреплённых адресов брать.
#
# Раньше брался первый попавшийся — и это вышло боком: закреплённых
# оказалось два, первым в списке лежал старый, который снаружи никто не
# видел, машина поднялась на нём. Теперь: если адрес назван явно — берём
# его; если закреплён ровно один — берём его; если несколько — не гадаем,
# а показываем список и просим выбрать.
RESERVED=$(yc vpc address list 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+' || true)
COUNT=$(printf '%s\n' "$RESERVED" | grep -c . || true)
NAT="${NAT_ADDRESS:-}"

if [ -n "$NAT" ]; then
  printf '%s\n' "$RESERVED" | grep -qx "$NAT" \
    || fail "адрес $NAT не найден среди закреплённых. Закреплены:
$(printf '%s\n' "$RESERVED" | sed 's/^/    /')"
elif [ "$COUNT" = "1" ]; then
  NAT="$RESERVED"
elif [ "${COUNT:-0}" -gt 1 ]; then
  fail "закреплённых адресов несколько, и я не знаю, какой ваш рабочий:

$(printf '%s\n' "$RESERVED" | sed 's/^/    /')

  Назовите нужный явно и запустите снова:
      NAT_ADDRESS=<адрес> bash create-vm.sh

  Лишние стоит удалить — за неиспользуемые закреплённые адреса
  Яндекс берёт больше, чем за рабочие:
      yc vpc address delete <имя-или-id>"
fi
...
if [ "$MODE" = "ready" ]; then
  ...
  # «|| true» здесь обязательно. Без него любая неудача внутри конвейера —
