...
async function applyMigrations(dbId) {
  ...
  for (const f of files) {
    ...
    /* Переносы строк выравниваем ДО срезания комментариев: на файле
       с виндовыми \r\n регулярка не срабатывает (точка не совпадает
       с \r), комментарий остаётся в выражении, и точка с запятой
       внутри него рвёт выражение пополам. На runner'е файлы приходят
       с \n, поэтому ошибка не всплывала — но всплыла бы при выкатке
       с рабочей машины. */
    const clean = raw
      .replace(/\r\n?/g, "\n")
      .split("\n")
