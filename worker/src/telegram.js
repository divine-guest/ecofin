...
export const TIPS = [
  ...
  "Документы храните не меньше пяти лет: налоговая может проверить и после закрытия бизнеса.",
...
Налог, взносы и остаток до лимита — ${siteUrl(env)}book.html`);
  ...
  try {
    ...
    await send(env, chatId, e.message === "paused"
      ? "Консультант временно недоступен — вернём в ближайшие дни. Напоминания и расчёты работают как обычно."
