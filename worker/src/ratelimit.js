/* ЭкоФин — ограничение частоты запросов.
...
export const LIMITS = {
  ...
  /* Сброс пароля: письма уходят с нашего адреса, а код — шесть цифр.
     Ограничиваем и адрес получателя, иначе чужой ящик можно засыпать
     письмами, а код — перебирать с разных IP. */
  reset:    { perIp: [10, 3600], perKey: [5, 3600] },
  /* Аварийное восстановление владельца. Здесь лимит жёстче всего остального,
     и вот почему: этот единственный запрос отдаёт полный контроль над
     сервисом — меняет пароль владельца и ставит роль owner. Пока лимита не
     было, ключ подбирался со скоростью 400 попыток в секунду, и стойкость
     всего сервиса упиралась в длину одной строки в настройках.
     5 попыток в час — человеку, который вводит свой же ключ, хватает
     с запасом, а перебору не хватит и за годы. */
  recover:  { perIp: [5, 3600], perKey: [5, 3600] },
};
