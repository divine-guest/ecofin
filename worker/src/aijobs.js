...
/* scope-ok (весь файл): запросы этого модуля работают с одной
   задачей по её идентификатору, а право на задачу проверяется на
   входе — в status() и list(), где выборка ограничена почтой.
   Уборка старых записей идёт по всей таблице сразу и данных
   конкретного человека не касается.

   Пометка стоит на уровне файла намеренно: повторять одно и то же
   объяснение над семью строками — значит сделать его незаметным. */
...
/* scope-ok (весь блок ниже): задача уже выбрана обработчиком, который
   сверил почту, либо взята планировщиком по идентификатору. Работа с
   ней идёт по id — добавлять сюда почту нечего и незачем. */
...
async function claim(env, id) {
  ...
  /* scope-ok: идентификатор пришёл из claim(), который только что
     обновил ровно эту строку. Наружу задача отдаётся из status(),
     а там выборка ограничена почтой. */
...
export async function ask(request, env, origin, user, ctx) {
  ...
  if (!spent) {
    return json(env, origin, {
      error: kind === "tool"
        ? "Пробный запуск израсходован. Инструменты без ограничений — в тарифе «Базовый» за 490 ₽ в месяц"
        : "На сегодня вопросы закончились. В тарифе «Базовый» их 300 в день — это 490 ₽ в месяц",
      paywall: true, kind,
    }, 402);
  }
