...
/* Зрячие модели, которым разрешено показывать изображение.

   Список закрытый и по одному признаку: государство должно входить
   в перечень, утверждённый приказом Роскомнадзора от 05.08.2022 № 128,
   и совпадать с тем, что указано в уведомлении о трансграничной
   передаче. Сегодня это Китай.

   Почему список, а не просто настройка. Изображение нельзя
   обезличить: в тексте мы заменяем имена и номера метками, а на
   снимке паспорта заменять нечего — уходит всё, включая подпись и
   печать. Цена ошибки в настройке здесь выше, чем где-либо ещё, а
   выглядит такая ошибка безобидно: вписали «gpt-4o, он лучше читает»
   и уехали в государство, которого нет в уведомлении.

   Добавлять сюда модель можно только вместе с проверкой государства
   по перечню и правкой уведомления. */
export const VISION_ALLOWED = {
  /* Zhipu AI (Z.ai), Китай. Буква «v» в названии — vision: это модели,
     сделанные под изображение, а не текстовые с довеском. */
  "glm-5v-turbo": "Китай",
  "glm-4.6v": "Китай",
  "glm-4.5v": "Китай",
  /* DeepSeek, Китай. Тот же получатель, что и у текстовой модели, —
     в уведомлении одна строка вместо двух. Но «exp» в названии значит
     экспериментальная: такая модель может исчезнуть из каталога без
     предупреждения, поэтому она не первый выбор. */
  "deepseek-v4-flash-vision-exp": "Китай",
  /* Qwen (Alibaba), Китай. Мультимодальные, но не специализированные
     на распознавании — запасной вариант. */
  "qwen3.8-flash": "Китай",
  "qwen3.8-max": "Китай",
};
